...
const FormAddMoney=():JSX.Element=>{
    //{id:1,title:'',type:'income',price:12000}
    const{Id}=useParams()
        ...
        <Grid container item xs={12} justifyContent={'center'}>
            <Grid container item xs={12}>
                <Grid item xs={12} md={4}p={1}>
                    <TextField select name={'type'} value={Form.type}fullWidth label={'هزینه یا درآمد'} onChange={handleChange} SelectProps={{native:true}}>
                        {options.map(item=>(
                ...
                <Grid item xs={12} md={4} p={1}>
                    <TextField fullWidth label={'موضوع'} value={Form.title} onChange={handleChange} name={'title'}></TextField>
                </Grid>
                <Grid item xs={12} md={4} p={1}>
                    <TextField fullWidth label={'مبلغ'} value={Form.price} onChange={handleChange} type={'number'} name={'price'}></TextField>
                </Grid>
            </Grid>
            <Grid item xs={12}>
                <Button fullWidth variant={'contained'} disabled={disabled} sx={{bgcolor:Form.type==='income'?'#942b57':'#942b2f'}} onClick={handleSubmit}>
